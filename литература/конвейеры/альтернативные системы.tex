\chapter{Альтернативные системы}

\section{Медленные системы}
...
Работает данная схема так: если $q_1 \ge q/2$ -- то разделитель работает по обычным правилам. В противном же случае появляется новое правило: разделитель первый цикл работает как $C[t_i]$ вплоть до посещения "критического" выхода. Как только до него доходят -- запускается таймер на $\frac1{q_1}$, и пока он не истечёт -- читается, что на этом выходе -- тупик - то есть $C[t_i] \to C[t_i-1]$
...
\begin{minipage}{0.5\textwidth}
	\begin{tikzpicture}[
		...
		\draw[r2]	(a.south)	to	node[right]	{$60$}	node[right]	{$60$}	(s1.north);
