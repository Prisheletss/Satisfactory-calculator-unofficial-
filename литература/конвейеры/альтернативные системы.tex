...
\begin{minipage}{0.3\textwidth}
	\begin{tikzpicture}[
		...
		\node[o]	(out1)	at	(-2, -2)	{$q_1-x$};
		\node[o]	(out2)	at	(0, -2)		{$x+y$};
		\node[o]	(out3)	at	(2, -2)		{$q_2-y$};
		
		
		
		% === === Lines === ===
		\draw[r]	(in1)	to	node[right]	{$q_1$}		(s1);
		\draw[r]	(in2)	to	node[right]	{$a_2$}		(s2);
		\draw[r]	(s1)	to	node[right]	{$q_1-x$}	(out1);
		\draw[r]	(s2)	to	node[right]	{$q_2-y$}	(out3);
		\draw[r]	(s1)	to	node[above]	{$x$}		(tri1);
		\draw[r]	(s2)	to	node[above]	{$y$}		(tri2);
		\draw[r2]	(a)		to	node[right]	{$x+y$}		(out2);
