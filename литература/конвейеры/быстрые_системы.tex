...
Тогда наше обозначение $\frac{a}{s} - \frac{b}{s} := \frac{a}{s-b}$ приобретает новый смысл: мы из периода $s$ нашей циклической группы вычитаем $b$:

$$\frac{a}{s} - \frac{b}{s} := \frac{a}{s-b} \ \equiv \ C[s] - C[b] := C[s-b]$$

\

\noindent Тогда нетрудно заметить, что выполняется линейность:

$$C[\alpha x + \beta y] = \alpha C[x] + \beta C[y]$$

\

А в связи с \underline{теоремой о делимости}, множество $\mathbb{C}$ всех циклических групп, которые мы можем построить: $\mathbb{C} = \{ C[n] \ | \ n \in \mathbb{N} \}$
...
\begin{align*}
	&\forall k \in \mathbb{N} \ : \ C[n] \in \mathbb{C} \ \Longrightarrow \ k\cdot C[n] \in \mathbb{C} \\
	&\forall C[a], C[b] \in \mathbb{C} \ : \ C[a] + C[b] \in \mathbb{C}
\end{align*}

\

\noindent Стало быть, $\mathbb{C}$ -- векторное пространство над полем $\mathbb{N}$, а все $C[n]$ -- вектора
