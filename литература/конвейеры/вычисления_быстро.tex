...
\footnote{Данная система является примером группы $\mathbb{Z}_2 \times \mathbb{Z}_3 = \mathbb{Z}_6$, в которой 6-ой элемент является и 0-ым}.
...
\begin{minipage}{0.2\textwidth}
	\begin{tikzpicture}[
		...
		\node[o]	(out2)	at	(-1, -2)	{$\uparrow$};
		
		%Lines
		\draw[r]	(in.south)	to	node[right]	{$60$}	(a1.north);
		\draw[r]	(a1.south)	to	node[right]	{$72$}	(s1.north);
		\draw[r]	(s1.south)	to	node[right]	{$36$}	(s2.north);
		\draw[r]	(s2.east)	-| (1, 1) node[right] {$12$} |- (a1.east);
	\end{tikzpicture}
...
\begin{minipage}{0.2\textwidth}
	\begin{tikzpicture}[
		...
		\draw[r]	(in.south) 	to	node[right]	{$60$}	(s1.north);
		\draw[r]	(s1.south)	to	node[right]	{$30$}	(s2.north);
		\draw[r]	(s1.east)	-|	node[above]	{$30$}	(a.north);
		\draw[r]	(s2.east)	to	node[above]	{$15$}	(a.west);
		\draw[r]	(s2.south)	to	node[left]	{$15$}	(out1.north);
		\draw[r]	(a.south)	to	node[right]	{$45$}	(out2.north);
	\end{tikzpicture}
